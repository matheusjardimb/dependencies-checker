...
function checkDependencyList(
  ...
  const dependencyBlock: {[index: string]: string} = packageJson[
    dependencyBlockKey
  ] as {}
...
function isDependencyBlock(keyName: string): boolean {
  ...
  return (
    (keyNameLower !== libSettingsKey.toLowerCase() &&
...
function getIgnoredDependencies(packageJson: {
  ...
  const libSettingsValue = packageJson[libSettingsKey]
  if (libSettingsValue !== undefined) {
    const ignoredDependencies = libSettingsValue[
      ignoredDependenciesKey
    ] as string[]
...
function read_package_json_file(
  packageJsonPath: string
): [string, {[p: string]: undefined}] {
...
function checkDependencies(packageJsonPath: string): void {
  ...
  if (packageJson[libSettingsKey] === undefined) {
    core.info(
      `Custom '${libSettingsKey}' block not informed, using default values`
    )
  }
